...
export default function EmployeeCard({ 
  ...
  return (
    <div className={`bg-gradient-to-r ${isBirthday ? 'from-pink-50 to-purple-50' : 'from-green-50 to-blue-50'} rounded-lg p-4 mb-3`}>
      <h5 className={`font-bold text-base sm:text-lg ${isBirthday ? 'text-purple-800' : 'text-blue-800'}`}>
        {employee.name}
      </h5>
      
      {isBirthday ? (
        <>
          <p className="text-xs sm:text-sm text-gray-600">🎂 Age: {calculateAge(employee.birthday)} years</p>
      ...
      <div className={`mt-3 p-3 bg-white rounded border-l-4 ${isBirthday ? 'border-pink-500' : 'border-green-500'}`}>
        <p className={`text-xs sm:text-sm font-medium ${isBirthday ? 'text-pink-700' : 'text-green-700'}`}>
